...
const index = () => {
  ...
  const renderItem = ({ item }) => {
    return (
      ...
      >
        <Image
          source={{ uri: constants.IMAGE_URL + item }}
          style={{
            width: "90%",
            height: 250,
            borderRadius: 10,
  ...
  const updateUserDesc = async () => {
    try {
      const res = await axios.put(constants.API_URL + `users/${userId}/desc`, {
        desc: desc,
      });
      if (res.status !== 200) {
    ...
    if (turnOns.includes(name)) {
      try {
        const res = await axios.put(
          constants.API_URL + `users/${userId}/turn-ons/remove`,
          {
            turnOns: name,
          }
        );
      ...
      try {
        const res = await axios.put(
          constants.API_URL + `users/${userId}/turn-ons/add`,
          {
            turnOns: name,
          }
        );
  ...
  const handleLookingFor = (name) => async () => {
    if (lookingFor.includes(name)) {
      try {
        const res = await axios.put(
          constants.API_URL + `users/${userId}/looking-for/remove`,
          {
            lookingFor: name,
          }
        );
      ...
      try {
        const res = await axios.put(
          constants.API_URL + `users/${userId}/looking-for/add`,
          {
            lookingFor: name,
          }
        );
  ...
  const getRandomImage = () => {
    ...
    return constants.IMAGE_URL + profileImages[randomIndex];
  };
  ...
  const uploadImage = async (uri) => {
    const url = constants.API_URL + `users/${userId}/profile-images`;
    const headers = {
      httpMethod: "POST",
  ...
  return (
    <ScrollView>
        ...
        <View>
          <View>
            <Pressable style={styles.container}>
              <Image source={{ uri: randomImage }} style={styles.logoStyle} />
      ...
      >
        {option === "Turn-ons" && (
          <>
            {constants.TURNON?.map((item, index) => {
              return (
                <Pressable
      ...
      >
        {option === "Looking For" && (
          ...
          >
            {constants.LOOKING_FOR?.map((item, index) => {
              return (
                <Pressable
                  key={index}
